...
const CategorySelectionModal: React.FC<CategorySelectionModalProps> = ({ onClose, onSelect, currentCategory, currentSubCategory }) => {
  ...
  const activeCategories = React.useMemo(() => {
    return (Object.keys(categories) as ReportCategory[]).filter(key => categories[key].is_active);
  }, [categories]);
      ...
      <div className="bg-card dark:bg-surface-dark rounded-2xl p-6 w-full max-w-lg m-4 flex flex-col relative max-h-[80vh]" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="overflow-y-auto space-y-2 pr-2">
          {activeCategories.map(categoryKey => {
            ...
            const categoryColor = theme === 'dark' ? category.color.dark : category.color.light;
                ...
                {isOpen && (
                  <div className="px-4 pb-4 space-y-1">
                    {Object.keys(category.subCategories).map(subCategoryKey => {
                      ...
                      const isSelected = currentCategory === categoryKey && currentSubCategory === subCategoryKey;
                      return (
                        <button
                          key={subCategoryKey}
                          onClick={() => handleSelect(categoryKey, subCategoryKey)}
