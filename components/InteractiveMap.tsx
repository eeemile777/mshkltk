...
import { Report, Theme, TimeFilter, ReportCategory, ReportSeverity } from '../types';
...
const InteractiveMap: React.FC<InteractiveMapProps> = ({
    ...
    const {
      ...
      activeCategories, activeStatuses, activeTimeFilter, t, language
    ...
    React.useEffect(() => {
        ...
        if (bounds) {
            // FIX: The `bounds` prop is already a LatLngBoundsExpression.
            // Leaflet's map options accept this type directly, so we don't need to re-wrap it with L.latLngBounds(),
            // which was causing a type error because L.latLngBounds() doesn't accept an existing LatLngBounds object.
            ...
            const pinsHtml = pinsToShow.map((marker: any, i: number) => {
                ...
                return `
                    <div class="fan-pin-wrapper" style="transform: rotate(${angle}deg); z-index: ${zIndex};">
                        <div class="report-leaflet-div-icon ${theme}" style="background-color: ${color}; width: 36px; height: 36px; border-width: 3px;">
                            <div class="icon-inner">${iconInnerHtml}</div>
                        </div>
                    </div>
                `;
            }).join('');
    ...
    React.useEffect(() => {
        ...
        filteredReports.forEach(report => {
            const icon = createCategoryIcon(report.category, theme);
    ...
    }, [filteredReports, theme, t, navigate, reportPathPrefix, language]);
