...
import { CATEGORIES, PATHS, translations } from '../constants';
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      img.decode()
        .then(async () => {
          requestAnimationFrame(async () => {
            if (img.naturalHeight === 0 || img.naturalWidth === 0) {
              return reject(new Error('Image decoded but has zero dimensions.'));
            }
            try {
              // 1) Background cover
        ...
        .catch(err => reject(new Error(`Image decoding failed: ${err}`)));
    };
...
const ShareModal: React.FC<ShareModalProps> = ({ report, onClose }) => {
  ...
  const downloadGeneratedImage = async () => {
    ...
    const dataUrl = await generateShareImage(report, translations, language, t[report.category], canvasFonts);
        ...
        <div className="w-full aspect-square rounded-xl overflow-hidden shadow-lg mb-6 relative bg-muted dark:bg-bg-dark">
          ...
          <div className="absolute bottom-4 left-4 right-4 text-white drop-shadow-lg">
            ...
            <div className="flex items-center gap-2 text-sm opacity-80">
              <CategoryIcon style={{ color: categoryColor }} /><span>{t[report.category]}</span><span className="mx-1">&bull;</span><FaMapLocationDot /><span>{report.area}</span>
            </div>
          </div>
        </div>
