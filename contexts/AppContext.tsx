...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const submitReport = React.useCallback(async (reportData) => {
    ...
    // Only update points and user stats for regular citizens in the main app flow
    if (!overrideUser && currentUser && currentUser.role === 'citizen') {
      const pointsForRule = gamificationSettings?.pointsRules.find(r => r.id === 'submit_report')?.points || 0;
      let updatedUser = {...currentUser, points: currentUser.points + pointsForRule, reports_count: currentUser.reports_count + 1};
      await handleUserUpdate(updatedUser);
      processBadgeAwards(updatedUser, [reportData as Report, ...reports], dynamicBadges);
    }
    ...
    return await queueForSync(submissionPayload);
  }, [effectiveCurrentUser, overrideUser, currentUser, handleUserUpdate, reports, processBadgeAwards, language, gamificationSettings, dynamicBadges]);
  ...
  const confirmReport = React.useCallback(async (reportId: string) => {
    ...
    // Only update points if it's a regular citizen
    if (!overrideUser && currentUser && currentUser.role === 'citizen') {
      const pointsForRule = gamificationSettings?.pointsRules.find(r => r.id === 'confirm_report')?.points || 0;
      const userAfterConfirm = { ...currentUser, points: currentUser.points + pointsForRule, reportsConfirmed: currentUser.reportsConfirmed + 1, confirmedReportIds: [...(currentUser.confirmedReportIds || []), reportId] };
      await handleUserUpdate(userAfterConfirm);
      processBadgeAwards(userAfterConfirm, reports, dynamicBadges);
    }
  }, [effectiveCurrentUser, overrideUser, currentUser, reports, updateReportInState, handleUserUpdate, processBadgeAwards, gamificationSettings, dynamicBadges]);
