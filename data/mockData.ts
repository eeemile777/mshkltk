...
const initialReports: Report[] = [
  {
    id: 'report-1',
    title_en: 'Large Pothole on Main Street',
    title_ar: 'حفرة كبيرة في الشارع الرئيسي',
    photo_urls: [getReportImageUrl('infrastructure'), getReportImageUrl('infrastructure')],
    lat: 33.8938,
    lng: 35.5018,
  ...
  {
    id: 'report-2',
    title_en: 'Overflowing Dumpster',
    title_ar: 'حاوية قمامة فائضة',
    photo_urls: [getReportImageUrl('waste_environment')],
    lat: 33.8886,
    lng: 35.4955,
    area: 'Hamra, Beirut',
    municipality: 'beirut',
  ...
  {
    id: 'report-3',
    title_en: 'Streetlight Out on Seaside Road',
    title_ar: 'ضوء شارع معطل على الطريق الساحلي',
    photo_urls: [getReportImageUrl('electricity_energy'), getReportImageUrl('electricity_energy')],
    lat: 34.4363,
    lng: 35.8444,
    area: 'Mina, Tripoli',
    ...
    photo_urls: [getReportImageUrl('electricity_energy')],
...
const allCategories = Object.keys(CATEGORIES) as ReportCategory[];
const allUsers = [
...
const generateReport = (id: number, city: City) => {
    ...
    const report: Report = {
        id: `report-${1000 + id}`,
        title_en: `${category.name_en} issue in ${city.name_en}`,
        title_ar: `مشكلة ${category.name_ar} في ${city.name_ar}`,
        photo_urls: [getReportImageUrl(categoryKey)],
        ...
        note_en: `This is a report regarding '${subCategory.name_en}' which has been observed in the area of ${city.name_en}. The situation requires attention.`,
        note_ar: `هذا بلاغ بخصوص '${subCategory.name_ar}' تم ملاحظته في منطقة ${city.name_ar}. الوضع يتطلب اهتماماً.`,
