...
import { CATEGORIES, STATUS_COLORS, PATHS, TILE_URLS } from '../constants';
                ...
                {reportHistory.map((item, index) => (
                     <div key={item.id} className="mb-6 last:mb-0">
                        ...
                        <p className="font-bold text-navy dark:text-text-primary-dark">
                            {item.status === ReportStatus.New ? t.reportCreated : `${t.statusChangedTo} "${t[item.status]}"`}
                        </p>
...
const MiniMap: React.FC<{ report: Report }> = ({ report }) => {
    ...
    React.useEffect(() => {
        ...
        if (tileLayer) {
            ...
            if (tilePane) {
                tilePane.style.filter = theme === Theme.DARK ? 'invert(100%) hue-rotate(180deg) brightness(95%) contrast(90%)' : '';
            }
        }
        
        const marker = markerRef.current;
        if (marker) {
            marker.setIcon(createCategoryIcon(report.category, theme));
        }
    }, [theme, report.category]);
...
const ReportDetailsPage: React.FC = () => {
  ...
  const { t, language, reports, loading, currentUser, confirmReport, fetchComments, fetchReportHistory, flyToLocation, toggleReportSubscription, theme } = React.useContext(AppContext);
  ...
  return (
    <div className="max-w-4xl mx-auto pb-20 md:pb-0">
      ...
      {lightboxState.isOpen && <Lightbox images={report.photo_urls} startIndex={lightboxState.startIndex} onClose={closeLightbox} />}
      {toastMessage && (
      ...
      <div className="bg-card dark:bg-surface-dark p-4 sm:p-6 rounded-2xl shadow-md">
        <ImageGrid report={report} onImageClick={openLightbox} />
