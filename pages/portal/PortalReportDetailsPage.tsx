...
import { FaArrowLeft, FaArrowRight, FaCircleCheck, FaRegCommentDots, FaClockRotateLeft, FaMapLocationDot, FaCity, FaGlobe, FaPaperPlane, FaSpinner, FaLandmark, FaCamera } from 'react-icons/fa6';
            ...
            <div className="relative pl-6 border-l-2 border-border-light dark:border-border-dark">
                {reportHistory.map(item => (
                     <div key={item.id} className="mb-6 last:mb-0">
                        <div className="absolute -left-[11px] w-5 h-5 bg-teal dark:bg-teal-dark rounded-full"></div>
                        <p className="font-bold text-navy dark:text-text-primary-dark">{item.status === ReportStatus.New ? t.reportCreated : `${t.statusChangedTo} "${t[item.status]}"`}</p>
                        <p className="text-xs text-text-secondary dark:text-text-secondary-dark mt-1">{new Date(item.updated_at).toLocaleString(t.language === 'ar' ? 'ar-LB' : 'en-US')}</p>
                    </div>
                ))}
            </div>
        </div>
...
const PortalReportDetailsPage: React.FC = () => {
  ...
  const { reports, users, loading, fetchCommentsForReport, fetchHistoryForReport, updateReportStatus, resolveReportWithProof } = React.useContext(PortalContext);
