...
const PortalReportsListPage: React.FC = () => {
    const { reports, loading, updateReportStatus, resolveReportWithProof } = React.useContext(PortalContext);
    ...
    }, []); // Run only on mount to capture incoming filter requests
        ...
        return (
            <th className={`p-4 font-bold text-text-secondary dark:text-text-secondary-dark ${className}`}>
                <button onClick={() => requestSort(sortKey)} className="flex items-center gap-2 hover:text-navy dark:hover:text-text-primary-dark transition-colors">
                    <span>{children}</span>
                    <Icon className={isSorted ? 'text-teal dark:text-teal-dark' : 'opacity-30'} />
                </button>
            </th>
        );
    };
    ...
    const handleExportCsv = () => {
        ...
        const rows = sortedAndFilteredReports.map(report => {
            ...
            return [
                report.id, report.title_en, report.title_ar, report.note_en, report.note_ar, t[report.category], t[report.sub_category || ''] || '', report.severity, report.status,
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-6">
                ...
                <div className="flex items-center gap-4 w-full md:w-auto">
                    <div className="relative flex-grow">
                        <FaMagnifyingGlass className="absolute top-1/2 -translate-y-1/2 left-3 text-text-secondary dark:text-text-secondary-dark" />
                        <input
                            type="search"
                                   ...
                                   <td className="p-4 align-top">
                                    {(() => {
                                        ...
                                        if (!categoryData) {
                                            return (
                                                <div>
                                                    <p className="font-semibold text-navy dark:text-text-primary-dark">{t[report.category]}</p>
                                                    <p className="block text-xs text-text-secondary dark:text-text-secondary-dark">{t[report.sub_category || '']}</p>
                                                </div>
                                            );
                                        }
