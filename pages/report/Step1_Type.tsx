...
import { FaCamera, FaPen, FaTriangleExclamation } from 'react-icons/fa6';
...
const NearbyReportCard: React.FC<{ report: Report & { distance: number } }> = ({ report }) => {
    ...
    const distanceText = report.distance < 1000
        ...
        : `${(report.distance / 1000).toFixed(1)}km away`;
        ...
        <Link to={PATHS.REPORT_DETAILS.replace(':id', report.id)} state={{ from: 'reportWizard' }} className="block bg-muted dark:bg-bg-dark p-3 rounded-xl hover:shadow-lg transition-shadow">
            <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-text-secondary dark:text-text-secondary-dark">{t[report.category]}</p>
...
const Step1Type: React.FC<Step1TypeProps> = ({ onSelect }) => {
    ...
    const renderDuplicateCheck = () => (
        <div className="w-full max-w-2xl text-center animate-fade-in">
            {/* FIX: Changed icon name from FaExclamationTriangle to FaTriangleExclamation to match react-icons/fa6 library. */}
            <FaTriangleExclamation className="text-5xl text-mango dark:text-mango-dark mx-auto mb-4" />
            <h2 className="text-2xl font-bold text-navy dark:text-text-primary-dark mb-2">Is this your issue?</h2>
            <p className="text-text-secondary dark:text-text-secondary-dark mb-6">We found these reports nearby. Confirming an existing issue helps us prioritize it faster!</p>
        ...
        <div className="animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-2xl">
                <button
                    ...
                    className="group flex flex-col items-center justify-center p-8 bg-card dark:bg-surface-dark rounded-2xl shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 border-2 border-transparent hover:border-teal"
                >
                    <FaCamera className="text-6xl text-teal dark:text-teal-dark mb-4 transition-transform duration-300 group-hover:scale-110" />
                    <h3 className="text-2xl font-bold text-navy dark:text-text-primary-dark">{t.reportWithMedia}</h3>
                ...
                <button
                    ...
                    className="group flex flex-col items-center justify-center p-8 bg-card dark:bg-surface-dark rounded-2xl shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 border-2 border-transparent hover:border-teal"
                >
                    <FaPen className="text-6xl text-sky dark:text-cyan-dark mb-4 transition-transform duration-300 group-hover:scale-110" />
                    <h3 className="text-2xl font-bold text-navy dark:text-text-primary-dark">{t.reportWithoutMedia}</h3>
