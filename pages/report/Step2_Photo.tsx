...
const getCorrectedImageFile = async (file: File): Promise<File> => {
  ...
  // No correction needed for orientation 1 (normal) or if orientation is not found/not applicable.
  if (orientation <= 1) {
    return file;
  }
  ...
  if (!ctx) return file; // Fallback if canvas context is not available
  ...
  return new Promise<File>((resolve) => {
    canvas.toBlob((blob) => {
      if (blob) {
        ...
        console.error('Canvas toBlob failed, falling back to original file.');
