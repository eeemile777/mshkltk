    ...
    const handler = setTimeout(() => {
      ...
      const areaSuggestions: SearchSuggestion[] = reports
        ...
        .filter((area, index, self) =>
          area && area.toLowerCase().includes(lowerCaseQuery) && self.indexOf(area) === index
        )
        .map(area => ({
          type: 'area',
          text: area,
              ...
              <div className="w-full max-w-lg relative">
                ...
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <span className="h-5 w-5 text-text-secondary dark:text-text-secondary-dark"><FaMagnifyingGlass aria-hidden="true" /></span>
                  </div>
                  <input
                    id="search"
                    name="search"
                  ...
                  {searchQuery && (
                    <div className="absolute inset-y-0 right-0 pr-3 flex items-center">
                      <button onClick={() => setSearchQuery('')} className="text-text-secondary dark:text-text-secondary-dark hover:text-text-primary dark:hover:text-text-primary-dark" aria-label="Clear search">
                        <span className="h-5 w-5"><FaXmark /></span>
                      </button>
                    </div>
                  )}
                </div>
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            {currentUser && (
              <>
                ...
                >
                  <span className="h-5 w-5"><FaBell /></span>
                ...
                <NavLink
                  ...
                  className="relative p-2 rounded-full text-text-secondary dark:text-text-secondary-dark hover:bg-muted dark:hover:bg-bg-dark"
                >
                  <span className="h-5 w-5"><FaTrophy /></span>
                </NavLink>
              </>
            )}
            {currentUser ? (
