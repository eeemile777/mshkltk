...
const Step3Location: React.FC<{
  ...
  setWizardStep,
    ...
    React.useEffect(() => {
      ...
      if (!reportData.location || reportData.municipality || isReverseGeocoding) {
        return;
      }
      const detectMunicipality = async () => {
        ...
        try {
          if (!reportData.location) {
            updateReportData({ municipality: 'beirut' });
            return;
          }
          ...
          const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3001'}/api/ai/detect-municipality`, {
            method: 'POST',
            headers: {
          ...
          if (!response.ok) {
            throw new Error('Municipality detection failed');
          }
    ...
    }, [reportData.location, reportData.municipality, isReverseGeocoding, updateReportData]);
    ...
    if (geoState.loading || !reportData.location) {
      return (
        <div className="flex flex-col items-center justify-center h-full w-full text-center p-4">
          <span className="animate-spin text-4xl text-teal dark:text-teal-dark mb-4"><FaSpinner{...({} as any)} /></span>
      ...
      <div className="flex flex-col h-full w-full">
        <div className="flex-grow min-h-0 overflow-y-auto pb-4 pr-1 space-y-4">
          ...
          <div className="p-4 bg-muted dark:bg-surface-dark rounded-xl flex items-start gap-4 text-start transition-shadow duration-300 animate-subtle-glow">
            ...
            <div className="relative w-full" ref={searchContainerRef}>
              ...
              <div className="relative">
                ...
                />
                {(isReverseGeocoding || isSearching) && <span className="animate-spin absolute top-1/2 -translate-y-1/2 right-2 text-teal dark:text-teal-dark"><FaSpinner{...({} as any)} /></span>}
              </div>
                ...
                <div className="absolute top-full mt-1 w-full bg-card dark:bg-surface-dark rounded-lg shadow-xl z-20 max-h-48 overflow-y-auto border border-border-light dark:border-border-dark">
                  {suggestions.map(s => (
                    <button key={s.place_id} onClick={() => handleSuggestionClick(s)} className="w-full text-left px-3 py-2 hover:bg-muted dark:hover:bg-bg-dark text-sm text-text-secondary dark:text-text-secondary-dark flex items-center gap-2">
                      <span className="flex-shrink-0"><FaMagnifyingGlass size={12}{...({} as any)} /></span> <span>{s.display_name}</span>
                    </button>
                  ))}
                </div>
              )}
          ...
          <div className="p-4 bg-muted dark:bg-surface-dark rounded-xl flex items-start gap-4 text-start">
            <span className="h-6 w-6 text-teal dark:text-teal-dark mt-1 flex-shrink-0"><FaCity{...({} as any)} /></span>
            <div>
              <label className="font-bold text-navy dark:text-text-primary-dark">{t.municipality}</label>
              {isDetectingMunicipality ? (
                <p className="text-sm text-text-secondary dark:text-text-secondary-dark flex items-center gap-2"><span className="animate-spin"><FaSpinner{...({} as any)} /></span> {t.aiAnalyzing}</p>
              ) : (
          ...
          <div className="p-4 bg-muted dark:bg-surface-dark rounded-xl flex items-start gap-4 text-start">
            <span className="h-6 w-6 text-teal dark:text-teal-dark mt-1 flex-shrink-0"><FaGlobe{...({} as any)} /></span>
            <div>
