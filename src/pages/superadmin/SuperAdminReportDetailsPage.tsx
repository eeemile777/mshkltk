...
const StatusTimeline: React.FC = () => {
    ...
    const getStatusIcon = (status: ReportStatus) => {
        switch (status) {
            ...
            case ReportStatus.InProgress: return <span className="animate-spin"><FaSpinner/></span>;
...
const SuperAdminReportDetailsPage: React.FC = () => {
    ...
    // Normalize lat/lng to numbers in case they come from database as strings
    ...
    const titleForConfirmation = language === 'ar' ? normalizedReport?.title_ar : normalizedReport?.title_en;
    ...
    const handleEditToggle = () => {
        if (isEditing) {
            ...
            if (normalizedReport) {
                setEditedReport({
                    ...normalizedReport,
                    lat: typeof normalizedReport.lat === 'string' ? parseFloat(normalizedReport.lat) : normalizedReport.lat,
                    lng: typeof normalizedReport.lng === 'string' ? parseFloat(normalizedReport.lng) : normalizedReport.lng,
                });
            }
            setIsEditing(true);
        }
    ...
    if (!normalizedReport) return <div className="text-center text-text-secondary dark:text-text-secondary-dark py-10">Report not found.</div>;

    const currentReportData = isEditing ? editedReport! : normalizedReport;
    ...
    return (
        <div className="max-w-4xl mx-auto">
            ...
            {lightboxState.isOpen && <Lightbox mediaUrls={normalizedReport.photo_urls} startIndex={lightboxState.startIndex} onClose={() => setLightboxState({isOpen: false, startIndex: 0})} />}

            <div className="bg-card dark:bg-surface-dark p-4 sm:p-6 rounded-2xl shadow-md">
                <MediaGrid report={normalizedReport} onMediaClick={(idx) => setLightboxState({isOpen: true, startIndex: idx})} />
                <div className="mt-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-3">
                            ...
                            <div className="p-3 bg-muted dark:bg-bg-dark rounded-xl space-y-2"><p className="text-text-secondary dark:text-text-secondary-dark">{currentReportData.area}</p><div className="text-sm text-text-secondary dark:text-text-secondary-dark flex items-center gap-2"><span className="flex-shrink-0 h-4 w-4 text-teal dark:text-teal-dark"><FaCity/></span><span className="capitalize"><span className="font-bold text-navy dark:text-text-primary-dark">{t.municipality}: </span>{currentReportData.municipality}</span></div><div className="text-sm text-text-secondary dark:text-text-secondary-dark flex items-center gap-2"><span className="flex-shrink-0 h-4 w-4 text-teal dark:text-teal-dark"><FaGlobe/></span><span className="font-mono">Lat: {currentReportData.lat.toFixed(5)}, Lng: {currentReportData.lng.toFixed(5)}</span></div></div>
                        ...
                        <div className="flex flex-col items-center justify-center bg-muted dark:bg-bg-dark p-6 rounded-xl text-center"><span className="text-5xl text-mango dark:text-mango-dark mb-3"><FaCircleCheck/></span><p className="text-3xl font-bold text-navy dark:text-text-primary-dark">{currentReportData.confirmations_count}</p><p className="text-text-secondary dark:text-text-secondary-dark">{t.confirmations}</p></div>
                    </div>
                    <div className="border-t border-border-light dark:border-border-dark my-6"></div>
