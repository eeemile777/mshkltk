...
test.describe('Portal - Complete Feature Testing', () => {
  ...
  test('3.7 - Change report status to In Progress', async ({ page }) => {
    ...
    if (isVisible) {
      ...
      const statusButton = page.locator('button:has-text("Change Status"), button:has-text("تغيير الحالة")');

      if (await statusSelect.isVisible()) {
        await statusSelect.selectOption({ value: 'in_progress' });
        // Save if needed
        ...
        console.log('ℹ️  Status change mechanism not found');
      }
